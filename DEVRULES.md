...
1. **Zero technical debt vs. `docs/KNOWN-GAPS.md`.** That document is, by construction, a register of deferred debt — thirteen open items, several explicitly deferred to future epics. Either the register is the sanctioned exception to the zero-debt rule (in which case say so) or the open items are a backlog to be burned down before new feature work.
