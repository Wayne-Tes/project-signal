...
- Wire to **real services only** — no ad-hoc stubs, placeholders, or one-off synthesised fixtures standing in for real behaviour in shipped code. Sanctioned exceptions, which are documented test infrastructure rather than fabrication:
  ...
  - **`apps/web/src/lib/data.ts` is NOT a sanctioned exception.** It is ~590 lines of generated mock data for a fictional bank, currently rendering in six dashboard views. It is pre-existing debt tracked as KNOWN-GAPS #13, and no new code may depend on it.
...
- **Console-driven fix loop until clean.** If anything misbehaves, is uncertain, or looks off: open the **browser console** (plus the network panel and the relevant server logs — the Next.js dev server output and `docker compose logs postgres pubsub`), read the ACTUAL errors, fix the root cause, and RE-TEST through the browser. Repeat until the issues are driven to zero and the console is clean. Never report an issue away, suppress it, or hand back a known-broken state.
...
| Local services         | `postgres`, `pubsub`                                               | `docker-compose.yml`                 |
...
1. ~~**Zero technical debt vs. `docs/KNOWN-GAPS.md`.**~~ **Resolved 2026-08-06 by the owner: KNOWN-GAPS is the backlog and is burned down before any new work.** It is not a sanctioned exception to the zero-debt rule. Two consequences: no new feature work — including the AWS migration — starts while items remain open, and any newly discovered defect is fixed on the spot rather than appended to the register. The single exception is a gap that a committed architectural decision _dissolves_ rather than defers; those are closed by the decision and marked as such, because paying them down on a platform being left is waste, not rigour.
...
3. **Wire to real services only vs. six dashboard views on mock data.** `apps/web/src/lib/data.ts` violates the rule today. It is KNOWN-GAPS #13 and Epic 6's exit criterion is its deletion.
