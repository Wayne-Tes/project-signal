...
| ~~1~~ | ~~Guardrails: tag defaults, name prefix, tag-filtered budget, teardown script~~       | ✅ **written 2026-08-07**, **hardened 2026-08-08** (account guard added to `00-discover.sh`; cost allocation tags moved to `infra-aws/account/`; CI gate added). `fmt`, `validate` and `shellcheck` green on all three root modules. **Apply pending SSO credentials** — see `infra-aws/README.md` |
