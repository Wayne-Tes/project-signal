...
| ~~1~~ | ~~Guardrails: tag defaults, name prefix, tag-filtered budget, teardown script~~       | ✅ **APPLIED 2026-08-08.** State bucket `psignal-dev-tfstate-290304998906` and budget `psignal-dev-monthly` ($150) both live and verified by reading AWS back, not state. ⛔ **Cost allocation tag activation is blocked** — linked accounts cannot manage it, so `10-preflight.sh` exits 1 and **gates Phase 2 by design**. See `AWS-SETUP.md` for the platform-team request |
| 2     | Foundation: VPC, RDS, S3, ECR, Secrets Manager                                        | Yes — **but see the gate below before starting**                                                                                                                                                                                                                                                                                                                              |
...
> ### ⛔ Phase 2 is gated on the platform team, and the gate is deliberate
>
> `10-preflight.sh` **exits 1** today. One resource carries the `Project` tag while the key is
> inactive, and that is precisely the silent-failure condition Phase 1 exists to detect: a
> tag-filtered budget on an inactive key reports a perfectly healthy **$0** while spend accrues.
>
> **Do not weaken the check to get past it.** The exposure right now is nil — the only tagged
> resource is an empty state bucket. It becomes real the moment Phase 2 creates RDS and Fargate,
> and because activation **does not backfill**, anything spent before the keys are active is
> unattributable permanently rather than retroactively fixable.
>
> The unblock is one action by the operator of management account `857154590661`, and the
> request is written out ready to send in [`AWS-SETUP.md`](AWS-SETUP.md) § Phase 1. Once they
> confirm, re-run the preflight: §3 should show all six keys `Active` and §6 should pass.
>
> **Everything in Phase 2 that does not spend money can proceed meanwhile** — writing the VPC,
> RDS and Secrets Manager Terraform, `fmt`/`validate`, and review. It is the _apply_ that waits.
